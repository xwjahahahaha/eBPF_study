...
# 根据宿主机上的容器进程pid获取容器ID
get_container_id() {
    # 注意：此方法只能获取容器1号进程的容器id TODO
    CONTAINERID=$(sudo docker inspect -f '{{.State.Pid}} {{.Id}}' $(sudo docker ps -q) | grep $PID | awk '{print $NF}')  # overlay2
}

get_container_merge_dir() {
    # 在宿主上获取容器目录
...
get_container_work_dir() {
    ...
    if [ -z $WORKDIR ]; then
        echo "ERROR: get container $WORKDIR WORKDIR for failed"
        exit 1
    fi
}

# 复制ibasyncProfiler.so文件到容器环境
copy_so_to_container() {
    get_container_merge_dir
    get_container_work_dir
    cp -f $PROFILER $MERGEDIR$WORKDIR/libasyncProfiler.so
}
...
jattach() {
    ...
    if [ ! -z $CONTAINERID ]; then
        # 如果是容器环境就复制到容器内
        copy_so_to_container 
        "$JATTACH" "$PID" load "./libasyncProfiler.so" true "$1,log=$LOG" 
    else
    ...
    if [ $RET -ne 0 ]; then
        ...
        #     rm -rf $MERGEDIR$WORKDIR/libasyncProfiler.so
        # fi
        rm -rf $MERGEDIR$WORKDIR/libasyncProfiler.so
        exit $RET
    fi
